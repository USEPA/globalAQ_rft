...
countryresults <-
   read_csv(file.path(inputs,"npd_all_country_means.csv"),
           col_types = cols()) %>%
...
globalresults <- 
  globalresults  %>%
  group_by_at(.vars = c('Model','Pollutant','discount.rate')) %>%
  mutate(npd = mean_npd, 
         npd_crf_2.5 = mean_npd_2.5,
         npd_crf_97.5 = mean_npd_97.5) %>%
  select(c(Model,discount.rate,Pollutant,npd, npd_crf_2.5, npd_crf_97.5)) %>%
  arrange(desc(npd)) %>%
  #filter(Model =='MMM') %>%
...
cResultslong <- countryresults %>%
  group_by_at(.vars = c('Model','Pollutant','SuperRegion','discount.rate')) %>% #add pollutant
  summarise_at(c('npd','npd_crf_2.5','npd_crf_97.5'),sum,na.rm=T) %>%
  ungroup() %>%
  arrange(desc(npd)) %>%
  #filter(Model =='MMM') #%>%
  rbind(globalresults) %>%
  #group_by_at(.vars = c('Model','SuperRegion','discount.rate')) %>%
  #summarise_at(c('npd','npd_crf_2.5','npd_crf_97.5'),sum,na.rm=T) %>% #sum PM+O3
  group_by_at(.vars = c('Pollutant','SuperRegion','discount.rate')) %>%
  summarise_at(c('npd','npd_crf_2.5','npd_crf_97.5'),mean,na.rm=T) #avg models
...
lower = cResultslong %>% 
  group_by_at(c('Pollutant','discount.rate','SuperRegion')) %>% 
  summarise(means=npd_crf_2.5) %>% 
  ungroup() %>%
  mutate(across(c('means'), ~ signif(.,2)))

upper = cResultslong %>% 
  group_by_at(c('Pollutant','discount.rate','SuperRegion')) %>% 
  summarise(means=npd_crf_97.5) %>% 
  ungroup() %>%
  mutate(across(c('means'), ~ signif(.,2)))
  ...
  summarize_at(.vars=c("npd"),sum,na.rm=T) %>%
  ungroup %>% arrange((npd)) %>%
  ...
  p1 <-  ggplot(cResultslong, aes(x=SuperRegion, y=npd))+
        geom_linerange(aes(x=SuperRegion,ymin=npd_crf_2.5, ymax=npd_crf_97.5,group=discount.rate),
...
p2 <-  cResultslong %>%
        ...
        ggplot(aes(x=SuperRegion, y=npd,fill = Pollutant))+
        geom_bar(stat = 'identity', position = 'dodge',width=0.7)+
        geom_pointrange(aes(x=SuperRegion,ymin=npd_crf_2.5, ymax=npd_crf_97.5),
                        position =
        ...
        scale_x_discrete(labels = function(x) str_wrap(x,width=30))+
        scale_y_continuous(limits=c(-35,30), label = scales::dollar)+
        xlab("")+ylab("$(2020)/mT" ~CO[2])+
        ggtitle(paste0('Damages per metric ton CO2 \nby GBD Super Region (Model Average)'))+
        theme_minimal()+
...
ggsave(plot = p2, filename = 
         file.path(Plots,'FigXX.png'),width = 10,height = 5)
