...
###SET
NAME = '8001_9000'
damages <- read_parquet(file.path(Outputs,'damages_8001_9000.parquet'))
...
rates = tibble(rate = c('2.0% Ramsey'),
               rho  = c(exp(0.001972641)-1), ## under discrete time, need to transform the rho that was calibrated using continuous time 
               #rho = c(0.000091496, 0.001972641, 0.004618785, 0.007702711, 0.02, 0.03),
               eta  = c( 1.244459020))
...
for (COUNTRY in 1:nrow(countries)) {
  ...
  if (allRFF ==1){
    data %>%
      write_parquet(file.path(Outputs, paste0('npd_full_streams_rff_',NAME,'_',countries$LocID[COUNTRY],'.parquet')))
  } else {
    data %>%
      write_parquet(file.path(Outputs, paste0('npd_full_streams_',NAME,'_',countries$LocID[COUNTRY],'.parquet')))
  }
  ...
  if (allRFF ==1){
    means %>%
      write_csv(file.path(Outputs,paste0('npd_country_rff_means_',NAME,'_',countries$LocID[COUNTRY],'.csv')))
    net_means %>%
      write_csv(file.path(Outputs,paste0('npd_country_rff_net_means_',NAME,'_',countries$LocID[COUNTRY],'.csv')))
  } else {
    means %>%
...
if (allRFF ==1){
  Results_comb = 
    list.files(Outputs, pattern = paste0("npd_full_streams_rff_",NAME,'_'),full.names = T) %>% 
    map_df(~read_results(.))
  Results_comb_net = 
    list.files(Outputs, pattern = paste0("npd_full_streams_rff_",NAME,'_'),full.names = T) %>% 
